...
var g_auto_update_leftmenu = true;
var DATA_READY = {
statusReady: false,
...
function hasTwoAccount() {
getAjaxData('api/user/state-login', function ($xml) {
var ret = xml2object($xml);
if (ret.type == 'response') {
if (ret.response.accounts_number == 2) {
g_hasTwoAccountFlag = true;
}
}
}, {
sync: true
...
g_simcard_timer = setInterval(getSimcardStatusInterval, SIMCARD_STATUS_REFRESH);
});
}
...
saveAjaxData('api/app/privacynoticeinfo', xmlStr, function($xml) {
var ret = xml2object($xml);
if (ret.type == 'response') {
...
if (typeof(rets.response.extern_password_type) != 'undefined' && rets.response.extern_password_type == '1') {
g_scarm_login = true;
} else {
g_scarm_login = false;
}
if (typeof(rets.response.extern_password_type) != 'undefined') {
g_scarmwps_login = true;
} else {
g_scarmwps_login = false;
}
}
}, {
sync: true
...
if(current_href != "voicebusy") getPrivacyNoticeInfo();
}
...
break;
case "updatesettings":
checkGotoHome(g_module.online_update_enabled && checkObjectExist(g_PageUrlTree, "settings", "system", "updatesettings") && g_auto_update_leftmenu);
break;
case "logview":
...
var MenuNumber = 0;
if (current_href == 'pincoderequired' || current_href == 'pukrequired' || current_href == 'simlockrequired' || current_href == 'nocard' || current_href == 'connectionless' || current_href == 'opennewwindow' || current_href == 'privacynotice') {
...
if (g_module.online_update_enabled && checkLeftMenu(g_PageUrlTree.settings.system.updatesettings) && g_auto_update_leftmenu) {
...
if (g_module.local_update_enabled && checkLeftMenu(g_PageUrlTree.update.update_local) && g_restore_default_status != '1') {
...
$('#pravacy_policy').attr("href","http://consumer.huawei.com/minisite/legal/privacy/statement.htm?country=US&amp;language=en-US");
setPrivacyPolicyUrl('#pravacy_policy');
if (g_module.gdpr_enabled) {
$('#pravacy_policy').after("<a id='privacy_policy' href='../html/privacynotice.html' target='_blank' class='clr_gray' rel = 'noopener noreferrer' >" + IDS_common_privacy_policy + "</a>");
}
getAjaxData('api/device/device-feature-switch', function($xml) {
var ret = xml2object($xml);
if (ret.type == 'response') {
...
$('#footer a:last').after("<a id='copyright_notice' href='/html/OPEN_SOURCE_SOFTWARE_NOTICE.pdf' class='clr_gray'>"+IDS_copyright_notice+"</a><br/>");
}
}
});
var temp = copyrightReplace(common_copyright);
$('#statement_main').html('<table><tr><td><div class="pull-left giflogofooter"></div></td><td>&nbsp;' + temp + "</td></tr></table>");
...
$('.hotlinkstring').show();
hasTwoAccount();
if (g_restore_default_status == '1' && current_href != 'home' && current_href != 'update' && current_href != 'quicksetup' && current_href != 'pincoderequired' && current_href != 'pukrequired' && current_href != 'simlockrequired' && current_href != 'nocard' && current_href != 'cradleDisconnected' && current_href != 'commend' && current_href != 'opennewwindow' && current_href != 'privacynotice') {
gotoPageWithoutHistory('quicksetup.html');
return;
}
if (current_href == 'pincoderequired' || current_href == 'pukrequired' || current_href == 'simlockrequired' || current_href == 'nocard' || current_href == 'cradleDisconnected' || current_href == 'update_autorun' || current_href == 'voicebusy' || current_href == 'opennewwindow' || current_href == 'privacynotice') {
...
getDeviceStatus();
showFooter();
$('.button_wrapper:not(#login_btn,#connect_btn,#disconnect_btn,#cancel_btn,.pop_confirm_cust,.manual_update_span)').live('mouseover', function() {
...
if ($.inArray(current_href, forbiddenPageOnBridegList) > -1 && checkLeftMenu(g_PageUrlTree.settings.security.bridgemode) && g_module.bridge_enabled) {
...
$('.logo').after("<div class='help'><span><select id='lang' style='display:none'></select></span><span class = 'clr_bold_a' style='display:none'><a id=help_url href='#' target='_blank' rel = 'noopener noreferrer'><span id='help_span'></span></a></span><span><span class='username'  id='username_span' style='display:none'></span></span><span>&nbsp;</span><span><span class='logout'><span id='logout_span'></span></span></span></div>");
} else {
...
if ($.browser.msie) {
$('.help div').remove();
$('.help').append(" <span><select id='lang' style='display:none'></select></span><span class = 'clr_bold_a'><a id=help_url href='#' target='_blank' rel = 'noopener noreferrer'><span id='help_span'></span></a></span>" +
...
dialogHtml += "<div class='login_dialog QR_login_dialog' id='dialog'>";
dialogHtml += "    <div class='login_dialog_content QR_dialog_content'>";
dialogHtml += "        <div class='login_dialog_header QR_dialog_header'>";
...
dialogHtml += '        </div>';
dialogHtml += "        <div class='login_dialog_table QR_dialog_table'>";
...
dialogHtml += '               </div>';
dialogHtml += "               <div id='username_wrapper' class='QR_username'>";
dialogHtml += '                   <p>' + dialup_label_user_name + common_colon + '</p>';
dialogHtml += "                   <span><input type='text' class='input QR_input' id='username' maxlength='15' /></span>";
dialogHtml += '               </div>';
dialogHtml += "               <div id='login_password_wrapper' class='QR_login_password'>";
dialogHtml += '               <p>' + common_password + common_colon + '</p>';
dialogHtml += "                   <span><input  type='password'  class='input QR_input' id='password' maxlength='15'/>&nbsp;&nbsp;</span>";
dialogHtml += '               </div>';
dialogHtml += "               <div class='forget_password_tab'>";
dialogHtml += "                   <span id='forget_password_tab'>" + IDS_forget_password_tab + "</span>";
dialogHtml += '               </div>';
dialogHtml += "               <div id='forget_password_wrapper' class='QR_forget_password'>";
dialogHtml += "                   <p id='forget_password_tips' style='display:none;'>" + IDS_forget_password_tips + "</p>";
dialogHtml += '               </div>';
dialogHtml += '               </div>';
dialogHtml += "               <div id='QRcode'>";
dialogHtml += "               <span class='leftLine'></span>";
dialogHtml += "               <span id='QR_domestic' class='code' style='display:none'>";
dialogHtml += "                   <img src='../res/domestic.png'>";
dialogHtml += '               </span>';
dialogHtml += "               <span id='QR_foreign' class='code' style='display:none'>";
dialogHtml += "                   <img src='../res/foreign.png'>";
dialogHtml += '               </span>';
dialogHtml += '               <p class="QR_title">' + QR_HuaweiApp + '</p>';
dialogHtml += '               </div>';
dialogHtml += '        </div>';
dialogHtml += "        <div class='login_dialog_table_bottom QR_dialog_table_bottom'>";
...
var topVal = $('#initial_configuration_wrapper').height() + 44;
if (LANGUAGE_DATA.current_language == 'ar_sa' || LANGUAGE_DATA.current_language == 'he_il' || LANGUAGE_DATA.current_language == 'fa_fa'){
$('#QRcode').css({'top':'84px'});
} else {
$('#QRcode').css({'top':topVal+'px'});
}
}
...
$('#privacy_notice').attr('disabled','disabled');
...
function showConfirmAutoUdateDialog(content, contenttitle, okFunc, cancelFunc, removeable,removeFunc) {
content = display_SIMtoUIM(content);
if (DIALOG_UNREMOVE != removeable) {
$('#div_wrapper').remove();
$('.dialog').remove();
...
dialogHtml += "    <div class='dialog_content'>";
dialogHtml += "        <div class='dialog_header'>";
dialogHtml += "            <span class='dialog_header_left clr_white'>" + contenttitle + '</span>';
dialogHtml += '        </div>';
dialogHtml += "        <div class='dialog_table'>"+ content +'</div>';
dialogHtml += "        <div class='dialog_table_bottom'>";
dialogHtml += "            <div class='dialog_table_r'>";
if (cancelFunc != '' && cancelFunc != ' ' && cancelFunc != null) {
dialogHtml += "              <span class='button_wrapper pop_Cancel manual_update_span'>";
dialogHtml += "                  <div id='pop_Cancel' class='manual_update'>" + common_cancel_update + '</div>';
dialogHtml += '              </span>';
}
dialogHtml += "&nbsp;&nbsp;&nbsp;&nbsp;<span class='button_wrapper pop_confirm'>";
dialogHtml += "                  <input id='pop_confirm' class='button_dialog' type='button' value='"+common_ok_update+"'/></span>";
dialogHtml += '            </div>';
dialogHtml += '        </div>';
dialogHtml += '    </div>';
dialogHtml += '</div>';
...
function showAutoUdateDialog(content, contenttitle, okFunc, cancelFunc, removeable,removeFunc) {
content = display_SIMtoUIM(content);
var auto_update_1 = guide_autoupg_1.replace('%s',auto_update_guide);
var auto_update_2 = guide_autoupg_3.replace('%s1',auto_update_guide).replace('%s2',common_settings).replace('%s3',system_label_system).replace('%s4',IDS_update_settings);
if (DIALOG_UNREMOVE != removeable) {
$('#div_wrapper').remove();
$('.dialog').remove();
}
...
dialogHtml += "            <div>" + auto_update_1 + '</div>';
dialogHtml += "            <div>" + guide_autoupg_2 + '</div>';
dialogHtml += "            <div style='padding-top:5px'>" + auto_update_2 + '</div>';
dialogHtml += '        </div>';
dialogHtml += "        <div class='dialog_table_bottom'>";
dialogHtml += "            <div class='dialog_table_r'>";
if (cancelFunc != '' && cancelFunc != ' ' && cancelFunc != null) {
dialogHtml += "              <span class='button_wrapper pop_Cancel manual_update_span'>";
dialogHtml += "                  <div id='pop_Cancel' class='manual_update'>" + guide_autoupdate_close + '</div>';
dialogHtml += '              </span>';
}
dialogHtml += "&nbsp;&nbsp;&nbsp;&nbsp;<span class='button_wrapper pop_confirm'>";
dialogHtml += "                  <input id='pop_confirm' class='button_dialog' type='button' value='"+guide_autoupdate_continue+"'/></span>";
dialogHtml += '            </div>';
dialogHtml += '        </div>';
dialogHtml += '    </div>';
dialogHtml += '</div>';
...
var ret = xml2object($xml);
if ('response' == ret.type) {
g_basic_infos = ret.response;
g_restore_default_status = ret.response.autoupdate_restore_default_status;
...
if (g_basic_infos != null && typeof(g_restore_default_status) == 'undefined') {
getAjaxData('api/device/basic_information', function($xml) {
var ret = xml2object($xml);
if ('response' == ret.type) {
g_basic_infos = ret.response;
...
'apn_retry_enabled': g_moduleswitch.apn_retry_enabled == '1' ? true : false,
'gdpr_enabled': g_moduleswitch.gdpr_enabled == '1' ? true : false
};
...
var mt = $(".error_message").height()-16;
if (errormsg && idOfTextbox == "password" && mt>4) {
$(".forget_password_tab").css({"margin-top":mt + "px"});
} else {
$(".forget_password_tab").css({"margin-top":"4px"});
}
}
...
function clearDeviceStatusInterval() {
clearInterval(g_decive_timer);
g_decive_timer = null;
}
...
g_decive_timer = setInterval(getDeviceStatusInterval, g_feature.update_interval);
}
function getDeviceStatusInterval() {
...
function clearSimcardStatusInterval() {
clearInterval(g_simcard_timer);
g_simcard_timer = null;
}
function refreshSimcardStatus() {
if (g_simcard_timer) {
return;
}
getSimcardStatusInterval();
g_simcard_timer = setInterval(getSimcardStatusInterval, SIMCARD_STATUS_REFRESH);
}
function getSimcardStatusInterval() {
getAjaxData('api/monitoring/converged-status', function($xml) {
var ret = xml2object($xml);
if (ret.type == 'response') {
g_main_convergedStatus = ret.response;
...
if (g_module.cradle_enabled && checkValueIsNull(G_cradleStationStatus) && CRADLE_NETLINE_EXIST == G_cradleStationStatus.cradlestatus && ETHERNET_LAN_MODE != G_cradleStationStatus.connectionmode && (CRADLEAUTOMODE != G_cradleStationStatus.connectionmode)) {
...
if (g_auto_update) {
$('#menu_update a').live('click', needLogin);
}
}
}
}
...
function updateTooltipWidth(id, str) {
var len = str.length;
var api = $('#' + id).qtip('api');
if (!api.updateWidth) {
return;
...
updateTooltipWidth('sim_signal_gif', STATUS_BAR_ICON_STATUS.sim_signal_tooltip_state);
$('.qtip-station').find('.qtip-content').html('<b>' + STATUS_BAR_ICON_STATUS.station_tooltip_state + '</b>');
updateTooltipWidth('station_gif', STATUS_BAR_ICON_STATUS.station_tooltip_state);
$('.qtip-wan').find('.qtip-content').html('<b>' + STATUS_BAR_ICON_STATUS.wan_tooltip_state + '</b>');
updateTooltipWidth('wan_gif', STATUS_BAR_ICON_STATUS.wan_tooltip_state);
$('.qtip-wifi').find('.qtip-content').html('<b>' + STATUS_BAR_ICON_STATUS.wifi_tooltip_state + '</b>');
updateTooltipWidth('wifi_gif', STATUS_BAR_ICON_STATUS.wifi_tooltip_state);
$('.qtip-battery').find('.qtip-content').html('<b>' + STATUS_BAR_ICON_STATUS.battery_tooltip_state + '</b>');
updateTooltipWidth('battery_gif', STATUS_BAR_ICON_STATUS.battery_tooltip_state);
$('.qtip-indoor').find('.qtip-content').html('<b>' + STATUS_BAR_ICON_STATUS.wifi_indoor_tooltip_state + '</b>');
updateTooltipWidth('indoor', STATUS_BAR_ICON_STATUS.wifi_indoor_tooltip_state);
$('.qtip-sms').find('.qtip-content').html('<b>' + STATUS_BAR_ICON_STATUS.unread_sms_tooltip_state + '</b>');
updateTooltipWidth('tooltip_sms', STATUS_BAR_ICON_STATUS.unread_sms_tooltip_state);
}
...
g_user_manual_url = "../" + USER_MANUAL_PATH + "/" + current_language + "/" + USER_MANUAL_PATH + "/" + USER_MANUAL_FILE_NAME;
...
return "<a href='" + $1_href + "' style='text-decoration:underline;' target='_blank' rel = 'noopener noreferrer' onclick='javascript:stopBubble(event)'>" + $1 + "</a>";
});
}
